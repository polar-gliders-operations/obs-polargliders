...
if os.path.isfile('/home/isgiddy/share/www/data/SB1812D.csv') == False:
    ...
    for response in msg:
        if isinstance(response, tuple):
            ...
            if From == 'data@sailbuoy.no':
                ...
                df['Time'] = np.datetime64(datetime.strptime(str(df['Time'])[9:28], '%d/%m/%Y %H:%M:%S'))
                df = df.set_index('Time')
                
                if i == 1: # Save the first instant to have a starting point
                    df.to_csv('/home/isgiddy/share/www/data/SB1812D.csv')
                
                else:
                    df.to_csv('/home/isgiddy/share/www/data/SB1812D_latest.csv')
                    
                    latest = pd.read_csv('/home/isgiddy/share/www/data/SB1812D_latest.csv')
                    full = pd.read_csv('/home/isgiddy/share/www/data/SB1812D.csv')
                    ...
                    if latest.iloc[-1].Time not in full.Time.values:
                        ...
                        full.to_csv('/home/isgiddy/share/www/data/SB1812D.csv')
                    else:
                        continue
